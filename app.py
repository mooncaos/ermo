    ...
    if (player.get("map") == "costa_maravai" and
       ...
       (player.get("map") == "vilalbina" and player.get("y", 0) >= 24 and
        ...
        if f.get("prosperina") or player.get("map") == "vilalbina":
            if f.get("prosperina") and random.random() < 0.5:
                emit("toast", {"text": random.choice([
                    "🛶 Zé: 'Aposta na corrida de caranguejo de hoje? O ruivo tá voando.'",
                    "🛶 Zé: 'Remar é filosofia: quem para, a maré decide por ele.'",
                    "🛶 Zé: 'Vou morrer remando, forasteiro. A cama é pra quem desiste.'",
                    "🛶 Zé: 'O mar cobra de todo mundo. Eu pago em piada.'"])})
