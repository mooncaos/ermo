...
const INT_THEMES = {
  ...
  loja_barbearia: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#5aa9e0', kind:'loja'},
  loja_livraria: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#c9a842', kind:'loja'},
  loja_floricultura: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e05a8a', kind:'loja'},
  loja_alfaiataria: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#8a6a9a', kind:'loja'},
  loja_peixaria: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#4a7a8a', kind:'loja'},
  loja_queijaria: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e8d8a0', kind:'loja'},
  loja_doceria: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0865a', kind:'loja'},
  loja_chas: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#7a9a6a', kind:'loja'},
  loja_luthier: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#a8845c', kind:'loja'},
  loja_banca: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#c43e5a', kind:'loja'},
  casa_valmont: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#c9a842', kind:'marmore'},
  embaixada_ermo: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#4a5a7a', kind:'loja'},
  porao_sapopemba: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#3a8a5a', kind:'comum'},
  casa_leiloes: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e8c860', kind:'marmore'},
  lar_pontual: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0a850', kind:'quarto'},
  lar_clarice: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0a850', kind:'quarto'},
  lar_rosendo: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0a850', kind:'quarto'},
  lar_batista: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0a850', kind:'quarto'},
  lar_anselmo: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0a850', kind:'quarto'},
  lar_gemeas: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0a850', kind:'quarto'},
  lar_trovao: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0a850', kind:'quarto'},
  lar_firmina: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0a850', kind:'quarto'},
  lar_elviro: {f1:'#6b5638', f2:'#73603f', wall:'#33281c', acc:'#e0a850', kind:'quarto'},
  ...
  solar_prospera:    {f1:'#7a5f3e', f2:'#856a46', wall:'#2a2014', acc:'#d4af37', kind:'comum'},
  templo_estrelado:  {f1:'#1c1826', f2:'#242030', wall:'#100c1a', acc:'#e8c860', kind:'marmore'},
  templo_doze:       {f1:'#8a8274', f2:'#948c7e', wall:'#4a4438', acc:'#e0c060', kind:'marmore'},
  torre_alvorada:    {f1:'#6e6656', f2:'#787060', wall:'#38321f', acc:'#c9a842', kind:'comum'},
  oficina_ferreiro:    {f1:'#5c4c3a', f2:'#665542', wall:'#2c2218', acc:'#e05a3a', kind:'loja'},
  oficina_cozinheiro:  {f1:'#6b4f34', f2:'#75583a', wall:'#33241a', acc:'#f2a14e', kind:'loja'},
  oficina_joalheiro:   {f1:'#6b4f34', f2:'#75583a', wall:'#33241a', acc:'#b06ae0', kind:'loja'},
  oficina_alquimista:  {f1:'#565c44', f2:'#5f664c', wall:'#262c1c', acc:'#7ac06a', kind:'loja'},
  oficina_costureiro:  {f1:'#6b4f34', f2:'#75583a', wall:'#33241a', acc:'#e08ae0', kind:'loja'},
  oficina_carpinteiro: {f1:'#6b4f34', f2:'#75583a', wall:'#33241a', acc:'#c9a05a', kind:'loja'},
  oficina_coureiro:    {f1:'#665238', f2:'#705b3f', wall:'#302416', acc:'#a8845c', kind:'loja'},
  arena:               {f1:'#8a7a56', f2:'#93835f', wall:'#44381f', acc:'#c43e3e', kind:'comum'},
...
function drawInteriorTile(c, map, ch, px, py, ts, gx, gy){
  ...
  const marbleFloor = () => {
    ...
    if((gx + gy) % 2) { c.fillStyle = th.f2; c.fillRect(px, py, ts, ts); }
    c.strokeStyle = 'rgba(255,255,255,0.05)'; c.lineWidth = 1;      // veios
    c.beginPath();
    c.moveTo(px + ((gx*13)%ts), py);
    c.quadraticCurveTo(px + ts*0.6, py + ts*0.4, px + ((gy*17)%ts), py + ts);
    c.stroke();
    if((gx*7 + gy*11) % 5 === 0){                                    // brilho estelar
      c.fillStyle = 'rgba(255,244,210,0.10)';
      c.fillRect(px + (gx*5)%ts, py + (gy*9)%ts, 1.6, 1.6);
    }
  ...
  switch(ch){
    ...
    case 'j': {
      ...
      c.fillStyle = '#6db9f4';
      c.fillRect(px + ts*0.14, py + ts*0.12, ts*0.72, ts*0.62);
      c.fillStyle = 'rgba(255,255,255,0.35)';
      c.beginPath(); c.moveTo(px + ts*0.2, py + ts*0.6); c.lineTo(px + ts*0.5, py + ts*0.15);
      c.lineTo(px + ts*0.64, py + ts*0.15); c.lineTo(px + ts*0.3, py + ts*0.66); c.closePath(); c.fill();
      c.strokeStyle = '#2a2018'; c.lineWidth = 2;
      c.strokeRect(px + ts*0.14, py + ts*0.12, ts*0.72, ts*0.62);
      c.beginPath(); c.moveTo(px + ts*0.5, py + ts*0.12); c.lineTo(px + ts*0.5, py + ts*0.74);
      c.moveTo(px + ts*0.14, py + ts*0.43); c.lineTo(px + ts*0.86, py + ts*0.43); c.stroke();
      break;
    }
    case 'E': {                                        // ESTANTE cheia de livros
      floorOf();
      c.fillStyle = '#4a3826'; c.fillRect(px + 1, py, ts - 2, ts);
      const cores = ['#a84848','#4868a8','#4a8a58','#b08a3a','#7a5aa0'];
      for(let sh = 0; sh < 3; sh++){
        const sy = py + ts*0.10 + sh*ts*0.30;
        c.fillStyle = '#2e2214'; c.fillRect(px + 2, sy + ts*0.20, ts - 4, 2);
        for(let bkx = 0; bkx < 4; bkx++){
          c.fillStyle = cores[(gx + gy + sh + bkx) % 5];
          c.fillRect(px + ts*0.12 + bkx*ts*0.20, sy, ts*0.15, ts*0.20);
        }
      }
      break;
    }
    case 'o': {                                        // BARRIL
      floorOf();
      c.fillStyle = '#7a5434';
      c.beginPath(); c.ellipse(px + ts/2, py + ts*0.55, ts*0.32, ts*0.38, 0, 0, Math.PI*2); c.fill();
      c.strokeStyle = '#3e2a14'; c.lineWidth = 1.6;
      c.beginPath(); c.ellipse(px + ts/2, py + ts*0.55, ts*0.32, ts*0.38, 0, 0, Math.PI*2); c.stroke();
      c.beginPath(); c.moveTo(px + ts*0.18, py + ts*0.44); c.lineTo(px + ts*0.82, py + ts*0.44);
      c.moveTo(px + ts*0.18, py + ts*0.66); c.lineTo(px + ts*0.82, py + ts*0.66); c.stroke();
      c.fillStyle = 'rgba(255,255,255,0.12)';
      c.fillRect(px + ts*0.26, py + ts*0.28, ts*0.12, ts*0.4);
      break;
    }
    case '^': {                                        // BANCO de madeira
      floorOf();
      c.fillStyle = '#6a4a2a';
      c.fillRect(px + ts*0.10, py + ts*0.30, ts*0.80, ts*0.22);   // encosto
      c.fillRect(px + ts*0.10, py + ts*0.58, ts*0.80, ts*0.20);   // assento
      c.fillStyle = '#4a3018';
      c.fillRect(px + ts*0.14, py + ts*0.78, ts*0.10, ts*0.14);
      c.fillRect(px + ts*0.76, py + ts*0.78, ts*0.10, ts*0.14);
      break;
    }
    case '/': {                                        // friso de VITRAL no alto
      c.fillStyle = th.wall; c.fillRect(px, py, ts, ts);
      const vc = ['#e05a4e','#f2c14e','#5aa9e0','#7ac06a','#b06ae0'];
      c.fillStyle = vc[(gx*3 + gy) % 5];
      c.beginPath(); c.ellipse(px + ts/2, py + ts*0.5, ts*0.26, ts*0.36, 0, 0, Math.PI*2); c.fill();
      c.strokeStyle = 'rgba(20,14,8,0.7)'; c.lineWidth = 1.6; c.stroke();
      c.beginPath(); c.moveTo(px + ts/2, py + ts*0.14); c.lineTo(px + ts/2, py + ts*0.86);
      c.moveTo(px + ts*0.24, py + ts*0.5); c.lineTo(px + ts*0.76, py + ts*0.5); c.stroke();
      c.save(); c.globalCompositeOperation = 'lighter'; c.globalAlpha = 0.25;
      c.fillStyle = vc[(gx*3 + gy) % 5];
      c.beginPath(); c.arc(px + ts/2, py + ts*0.5, ts*0.5, 0, Math.PI*2); c.fill();
      c.restore();
      break;
    }
    case ';': {                                        // BRASEIRO aceso
      floorOf();
      c.fillStyle = '#3a3a42';
      c.fillRect(px + ts*0.32, py + ts*0.5, ts*0.36, ts*0.34);
      c.fillStyle = '#2a2a30';
      c.fillRect(px + ts*0.28, py + ts*0.46, ts*0.44, ts*0.08);
      const fl = (Date.now()/120) % 2;
      c.fillStyle = fl < 1 ? '#ff9a3a' : '#ffb85a';
      c.beginPath(); c.moveTo(px + ts*0.5, py + ts*0.16);
      c.quadraticCurveTo(px + ts*0.66, py + ts*0.36, px + ts*0.5, py + ts*0.5);
      c.quadraticCurveTo(px + ts*0.34, py + ts*0.36, px + ts*0.5, py + ts*0.16); c.fill();
      c.fillStyle = '#ffe08a';
...
function drawOutfitExtras(c, px, py, ts, look, facing, moving, walk){
  ...
  // ===== A ANATOMIA REAL DO BONECO (as mesmas fórmulas do corpo base) =====
  const cyc = ((walk||0) % WALK_CYCLE) / WALK_CYCLE;
  const bob = moving ? -Math.abs(Math.sin(cyc*Math.PI*2))*1.4 : 0;
  const cx = px + ts/2;
  const hr = ts*0.205, hy = py + ts*0.335 + bob;          // cabeça (centro/raio)
  const topY = hy - hr;                                    // topo do crânio
  const bodyTop = py + ts*0.42 + bob;
  const bodyH = ts*0.40, bodyW = ts*(fem ? 0.395 : 0.44);
  const bodyBot = bodyTop + bodyH;
  const beltY = bodyTop + bodyH*0.62;                      // linha do cinto
  const backX = cx + (facing === 'left' ? 1 : -1)*bodyW*0.62;
  const sway = Math.sin(now/300 + px)*ts*0.03;             // balanço de tecido
  const u = ts/32;                                         // unidade (1px @ts32)
  c.save();
  c.lineCap = 'round';

  // ---- helpers proporcionais ----
  function faixaPeito(cor){                                // faixa diagonal
    c.fillStyle = cor;
    c.beginPath();
    c.moveTo(cx - bodyW*0.46, bodyTop + bodyH*0.10);
    c.lineTo(cx + bodyW*0.34, bodyTop + bodyH*0.78);
    c.lineTo(cx + bodyW*0.46, bodyTop + bodyH*0.62);
    c.lineTo(cx - bodyW*0.30, bodyTop + bodyH*0.02);
    c.closePath(); c.fill();
  }
  function cinto(cor, fivela){
    c.fillStyle = cor;
    c.fillRect(cx - bodyW*0.5, beltY - u*1.2, bodyW, u*2.6);
    if(fivela){ c.fillStyle = fivela; c.fillRect(cx - u*1.6, beltY - u*1.6, u*3.2, u*3.4); }
  }
  function coleteAberto(cor){                              // duas abas laterais
    c.fillStyle = cor;
    c.fillRect(cx - bodyW*0.5, bodyTop, bodyW*0.26, bodyH*0.9);
    c.fillRect(cx + bodyW*0.24, bodyTop, bodyW*0.26, bodyH*0.9);
  }
  function chapeuAba(cor, abaR, copaH, det){               // chapéu de aba (palha/couro/bucket)
    c.fillStyle = cor;
    c.beginPath(); c.ellipse(cx, topY + u*1.2, abaR, abaR*0.38, 0, 0, Math.PI*2); c.fill();
    c.beginPath(); c.ellipse(cx, topY - copaH*0.45, hr*0.72, copaH*0.62, 0, 0, Math.PI*2); c.fill();
    c.strokeStyle = 'rgba(0,0,0,0.3)'; c.lineWidth = u;
    c.beginPath(); c.ellipse(cx, topY + u*1.2, abaR, abaR*0.38, 0, 0, Math.PI*2); c.stroke();
    if(det){ c.fillStyle = det; c.fillRect(cx - hr*0.72, topY - u*1.4, hr*1.44, u*1.6); }
  }
  function capuz(cor){                                     // capuz colado ao crânio
    c.fillStyle = cor;
    c.beginPath(); c.arc(cx, hy, hr*1.12, Math.PI*0.92, Math.PI*2.08); c.fill();
    c.beginPath();
    c.moveTo(cx - hr*1.05, hy + hr*0.30);
    c.quadraticCurveTo(cx, hy + hr*0.95, cx + hr*1.05, hy + hr*0.30);
  ...
  function armaCostas(tipo){
    if(tipo === 'arco'){
      ...
      c.beginPath(); c.arc(backX, bodyTop + bodyH*0.35, bodyH*0.62, -1.05, 1.05); c.stroke();
      c.strokeStyle = 'rgba(230,225,205,0.85)'; c.lineWidth = u*0.8;
      const r0 = bodyH*0.62, byy = bodyTop + bodyH*0.35;
      c.beginPath(); c.moveTo(backX + Math.cos(-1.05)*r0, byy + Math.sin(-1.05)*r0);
      c.lineTo(backX + Math.cos(1.05)*r0, byy + Math.sin(1.05)*r0); c.stroke();
    }
    if(tipo === 'vara'){
      c.save(); c.translate(backX, bodyTop + bodyH*0.4); c.rotate(-0.6);
      c.strokeStyle = '#8a6a3a'; c.lineWidth = u*1.6;
      c.beginPath(); c.moveTo(0, bodyH*0.35); c.lineTo(0, -bodyH*0.85); c.stroke();
      c.strokeStyle = 'rgba(220,230,240,0.7)'; c.lineWidth = u*0.7;
      c.beginPath(); c.moveTo(0, -bodyH*0.85);
      c.quadraticCurveTo(u*4, -bodyH*0.55, u*3, -bodyH*0.2); c.stroke();
      c.restore();
    }
    if(tipo === 'escudo'){
      const sy = bodyTop + bodyH*0.32, R = bodyW*0.42;
      c.fillStyle = '#3a4a5a';
      c.beginPath(); c.moveTo(backX, sy - R);
      c.lineTo(backX + R*0.9, sy - R*0.3); c.lineTo(backX + R*0.7, sy + R*0.7);
      c.lineTo(backX, sy + R*1.15); c.lineTo(backX - R*0.7, sy + R*0.7);
      c.lineTo(backX - R*0.9, sy - R*0.3); c.closePath(); c.fill();
      c.strokeStyle = 'rgba(0,0,0,0.4)'; c.lineWidth = u; c.stroke();
      c.fillStyle = '#c9b44a';
      c.beginPath(); c.arc(backX, sy + R*0.1, R*0.28, 0, Math.PI*2); c.fill();
    }
  }
  ...
  if(oid === 'aldeao'){
    ...
    if(fem){
      ...
      c.beginPath(); c.moveTo(cx + hr*0.8, hy);
      c.lineTo(cx + hr*1.5, hy + hr*0.6); c.lineTo(cx + hr*0.7, hy + hr*0.45); c.closePath(); c.fill();
      c.fillStyle = '#e8ddc2';                             // aventalzinho sobre a saia nativa
      c.beginPath();
      c.moveTo(cx - bodyW*0.24, beltY);
      c.lineTo(cx + bodyW*0.24, beltY);
      c.lineTo(cx + bodyW*0.18, py + ts*0.76 + bob);
      c.lineTo(cx - bodyW*0.18, py + ts*0.76 + bob);
      c.closePath(); c.fill();
    } else {
      c.strokeStyle = '#6a4a2a'; c.lineWidth = u*1.4;      // suspensórios
      c.beginPath(); c.moveTo(cx - bodyW*0.22, bodyTop); c.lineTo(cx - bodyW*0.18, beltY);
      c.moveTo(cx + bodyW*0.22, bodyTop); c.lineTo(cx + bodyW*0.18, beltY); c.stroke();
      cinto('#6a4a2a');
    }
    if(ads.includes(1)) chapeuAba('#d8b86a', hr*1.5, hr*0.8, fem ? '#c04a5a' : '#a8842e');
    if(ads.includes(2)){
      c.fillStyle = '#7a5a34';
      c.fillRect(backX - bodyW*0.20, bodyTop + bodyH*0.08, bodyW*0.40, bodyH*0.55);
      c.strokeStyle = '#4a3418'; c.lineWidth = u;
      c.strokeRect(backX - bodyW*0.20, bodyTop + bodyH*0.08, bodyW*0.40, bodyH*0.55);
      c.fillStyle = '#e8ddc2';
      c.fillRect(backX - u*1.2, bodyTop - u*1.5, u*2.4, u*3);
    }
  }
  ...
  if(oid === 'cacador'){
    ...
    if(ads.includes(1)){
      capuz('#3a462c');
      c.strokeStyle = '#1a1a12'; c.lineWidth = u*1.2;      // a pena
      c.beginPath(); c.moveTo(cx + hr*0.6, topY);
      c.quadraticCurveTo(cx + hr*1.5, topY - hr*0.8, cx + hr*1.8, topY + hr*0.2); c.stroke();
    }
    if(ads.includes(2)){
      c.save(); c.translate(backX, bodyTop + bodyH*0.15); c.rotate(0.5);
      c.fillStyle = '#6a4a2a'; c.fillRect(-u*2.2, 0, u*4.4, bodyH*0.62);
      c.strokeStyle = '#e8e2d0'; c.lineWidth = u*0.9;
      c.beginPath(); c.moveTo(-u, 0); c.lineTo(-u*2, -u*3.5);
      c.moveTo(u, 0); c.lineTo(u*0.4, -u*3.8); c.stroke();
      c.restore();
    }
  }
  ...
  if(oid === 'mercenario'){
    ...
    c.fillStyle = '#c9c4bc'; c.fillRect(-u*2.2, -u*1.8, u*4.4, u*1.8);
    c.fillStyle = '#8a6a2a'; c.fillRect(-u*0.8, -u*4, u*1.6, u*2.2);
    c.restore();
    if(!fem){
      c.fillStyle = '#8a3030';                             // bandana
      c.beginPath(); c.arc(cx, hy - hr*0.25, hr*1.04, Math.PI*1.0, Math.PI*2.0); c.fill();
      c.fillRect(cx + hr*0.7, hy - hr*0.35, hr*0.7, u*1.6);
    }
    if(ads.includes(1)){
      c.fillStyle = '#8a8a94';                             // ombreira
      c.beginPath(); c.ellipse(cx - bodyW*0.44, bodyTop + u, bodyW*0.30, u*2.8, -0.3, 0, Math.PI*2); c.fill();
      c.strokeStyle = '#3a3a42'; c.lineWidth = u*0.9; c.stroke();
    }
    if(ads.includes(2)){
      c.fillStyle = 'rgba(168,42,42,0.92)';                // capa do campeão
      c.beginPath();
      c.moveTo(backX - bodyW*0.28, bodyTop);
      c.quadraticCurveTo(backX - bodyW*0.42 + sway, bodyTop + bodyH*0.6, backX - bodyW*0.24 + sway, py + ts*0.80 + bob);
      c.lineTo(backX + bodyW*0.24 + sway, py + ts*0.80 + bob);
      c.quadraticCurveTo(backX + bodyW*0.42 + sway, bodyTop + bodyH*0.6, backX + bodyW*0.28, bodyTop);
      c.closePath(); c.fill();
      c.fillStyle = '#e0c98a';
      c.fillRect(backX - bodyW*0.28, bodyTop - u, bodyW*0.56, u*1.4);
    }
  }
  ...
  if(oid === 'aprendiz'){
    ...
    for(let i = 0; i < 3; i++){
      ...
      c.fillStyle = '#a89aff';
      c.fillRect(cx - bodyW*0.3 + i*bodyW*0.3, (fem ? py + ts*0.76 : bodyBot) - u*2 + bob, u*1.2, u*1.2);
    }
    c.restore();
    if(ads.includes(1)){
      c.fillStyle = '#4a4a7a';                             // chapéu pontudo (na cabeça!)
      const aba = hr*(fem ? 1.55 : 1.35);
      c.beginPath(); c.ellipse(cx, topY + u, aba, aba*0.35, 0, 0, Math.PI*2); c.fill();
      c.beginPath(); c.moveTo(cx - hr*0.8, topY + u*0.5);
      c.quadraticCurveTo(cx - hr*0.1, topY - hr*1.7, cx + hr*0.45, topY - hr*2.0);
      c.quadraticCurveTo(cx + hr*0.45, topY - hr*0.9, cx + hr*0.8, topY + u*0.5);
      c.closePath(); c.fill();
      c.fillStyle = '#8a7ae0'; c.fillRect(cx - hr*0.8, topY - u*1.2, hr*1.6, u*1.4);
    }
    if(ads.includes(2)){
      c.fillStyle = '#5a3a8a';                             // grimório no quadril
      c.fillRect(cx + bodyW*0.34, beltY + u, u*4.4, u*5.6);
      c.fillStyle = '#e8d8a0'; c.fillRect(cx + bodyW*0.34, beltY + u, u*1.1, u*5.6);
      c.strokeStyle = '#c9a842'; c.lineWidth = u*0.7;
      c.strokeRect(cx + bodyW*0.34, beltY + u, u*4.4, u*5.6);
    }
  }
  ...
  if(oid === 'pescador'){
    ...
    if(fem){ c.fillStyle = '#c04a5a';
      c.beginPath(); c.arc(cx, bodyTop + u*2, u*1.6, 0, Math.PI*2); c.fill(); }
    if(ads.includes(1)){
      chapeuAba('#4a6a5a', hr*1.45, hr*0.75, null);
      for(const [k, cor] of [[-1, '#e05a4e'], [0, '#f2c14e'], [1, '#5aa9e0']]){
        c.fillStyle = cor;
        c.beginPath();
        c.arc(cx + k*hr*1.1, topY + u*2.6 + Math.sin(now/300 + k)*u*0.6, u*1.2, 0, Math.PI*2);
        c.fill();
      }
    }
    if(ads.includes(2)){
      c.save(); c.translate(cx - bodyW*0.5, beltY + u*2); c.rotate(-0.9);
      c.fillStyle = '#d8b04c';
      c.beginPath(); c.ellipse(0, 0, u*3.8, u*1.7, 0, 0, Math.PI*2); c.fill();
      c.beginPath(); c.moveTo(u*3.3, 0); c.lineTo(u*5.6, -u*1.9); c.lineTo(u*5.6, u*1.9); c.closePath(); c.fill();
      c.restore();
    }
  }
  ...
  // ================= ANDARILHO =================
  if(oid === 'andarilho'){
    c.fillStyle = fem ? '#8a5a3a' : '#6a5a3a';             // O PONCHO (sobre o corpo real)
    c.beginPath();
    c.moveTo(cx, bodyTop - u*2);
    ...
    c.fillStyle = fem ? '#c04a5a' : '#8a3030';              // cachecol
    c.fillRect(cx - bodyW*0.30, bodyTop - u*2.6, bodyW*0.60, u*2.2);
    c.strokeStyle = '#6a4a24'; c.lineWidth = u*1.6;         // cajado
    const stX = cx + (facing === 'left' ? -1 : 1)*bodyW*0.62;
    c.beginPath(); c.moveTo(stX, bodyTop - u*3); c.lineTo(stX, py + ts*0.86 + bob); c.stroke();
    c.strokeStyle = '#8a6a3a'; c.lineWidth = u*1.1;
    c.beginPath(); c.arc(stX, bodyTop - u*4.2, u*1.8, 0.4, Math.PI*1.4); c.stroke();
    if(ads.includes(1)) chapeuAba('#6a4a2a', hr*1.6, hr*0.75, '#c9a05a');
    if(ads.includes(2)){
      const ly = beltY + u*4 + Math.sin(now/350)*u*0.8;     // lanterna acesa
      c.strokeStyle = '#3a3a42'; c.lineWidth = u;
      c.beginPath(); c.moveTo(cx + bodyW*0.40, beltY); c.lineTo(cx + bodyW*0.46, ly - u*1.6); c.stroke();
      c.fillStyle = '#2a2a32'; c.fillRect(cx + bodyW*0.40, ly - u*1.6, u*3, u*4.2);
      c.save(); c.globalCompositeOperation = 'lighter';
      const g = c.createRadialGradient(cx + bodyW*0.46, ly, 0, cx + bodyW*0.46, ly, u*6.5);
      g.addColorStop(0, 'rgba(255,206,110,0.9)'); g.addColorStop(1, 'rgba(0,0,0,0)');
      c.fillStyle = g; c.beginPath(); c.arc(cx + bodyW*0.46, ly, u*6.5, 0, Math.PI*2); c.fill();
      c.restore();
      c.fillStyle = '#ffe9b0'; c.fillRect(cx + bodyW*0.43, ly - u*0.8, u*1.6, u*2.4);
    }
  }
  ...
  if(oid === 'guardiao'){
    ...
    c.lineTo(cx + u*2, bodyTop + bodyH*0.38); c.closePath(); c.fill();
    armaCostas('escudo');
    if(ads.includes(1)){
      c.fillStyle = '#9aa0aa';                             // elmo
      c.beginPath(); c.arc(cx, hy - hr*0.1, hr*1.06, Math.PI*0.92, Math.PI*2.08); c.fill();
      c.fillRect(cx - u*0.8, hy - hr*0.2, u*1.6, hr*0.9);  // nasal
      c.fillStyle = fem ? '#e05a8a' : '#c43e3e';           // plumas
      for(let i = -1; i <= 1; i++){
        c.beginPath();
        c.ellipse(cx + i*u*2, topY - hr*0.7 + Math.abs(i)*u, u*1.3, hr*0.55, i*0.2, 0, Math.PI*2);
        c.fill();
      }
    }
    if(ads.includes(2)){
      c.strokeStyle = '#6a5a3a'; c.lineWidth = u*1.4;      // estandarte
      c.beginPath(); c.moveTo(backX, bodyTop); c.lineTo(backX, py - ts*0.16 + bob); c.stroke();
      c.fillStyle = '#c9b44a';
      c.beginPath(); c.moveTo(backX, py - ts*0.16 + bob);
      c.lineTo(backX + bodyW*0.5 + sway, py - ts*0.08 + bob);
      c.lineTo(backX, py + bob); c.closePath(); c.fill();
      c.fillStyle = '#3a4a5a';
      c.beginPath(); c.moveTo(backX + bodyW*0.16, py - ts*0.12 + bob);
      c.lineTo(backX + bodyW*0.32, py - ts*0.08 + bob);
      c.lineTo(backX + bodyW*0.16, py - ts*0.045 + bob); c.closePath(); c.fill();
    }
  }
  ...
  if(oid === 'mergulhador'){
    ...
    c.fillStyle = '#a06aff';
    c.fillRect(cx - bodyW*0.26, bodyTop + bodyH*0.18, u*1.5, u*1.5);
    c.fillRect(cx + bodyW*0.14, bodyTop + bodyH*0.42, u*1.5, u*1.5);
    c.fillRect(cx - u*0.7, beltY + u*3, u*1.5, u*1.5);
    c.globalAlpha = 0.4 + 0.2*Math.sin(now/300);           // orla de energia
    c.strokeStyle = '#8a5aff'; c.lineWidth = u;
    const barY = (fem ? py + ts*0.79 : bodyBot) + bob;
    c.beginPath(); c.moveTo(cx - bodyW*0.5, barY); c.lineTo(cx + bodyW*0.5, barY); c.stroke();
    c.globalAlpha = 0.55 + 0.3*Math.sin(now/500);          // olhos na sombra
    c.fillStyle = '#b088ff';
    c.fillRect(cx - hr*0.45, hy - u*0.6, u*1.4, u*1.1);
    c.fillRect(cx + hr*0.12, hy - u*0.6, u*1.4, u*1.1);
    c.restore();
    if(ads.includes(1)){
      c.fillStyle = '#3a2a5a';                             // máscara da Fenda
      c.fillRect(cx - hr*0.85, hy - u*1.1, hr*1.7, u*2.6);
      c.save(); c.globalCompositeOperation = 'lighter';
      c.globalAlpha = 0.75 + 0.25*Math.sin(now/260);
      c.fillStyle = '#c9a0ff';
      c.fillRect(cx - hr*0.55, hy - u*0.6, u*1.8, u*1.5);
      c.fillRect(cx + hr*0.1, hy - u*0.6, u*1.8, u*1.5);
      c.restore();
    }
    if(ads.includes(2)){
      c.save(); c.globalCompositeOperation = 'lighter';    // FRAGMENTOS ORBITAIS
      for(let i = 0; i < 3; i++){
        const a = now/800 + i*2.094;
        const ox = cx + Math.cos(a)*bodyW*0.85;
        const oy = bodyTop + bodyH*0.35 + Math.sin(a)*bodyH*0.35;
        c.globalAlpha = 0.9;
        c.fillStyle = '#a06aff';
        c.save(); c.translate(ox, oy); c.rotate(a*2);
        c.beginPath(); c.moveTo(0, -u*2.8); c.lineTo(u*2.1, 0); c.lineTo(0, u*2.8);
        c.lineTo(-u*2.1, 0); c.closePath(); c.fill();
        c.restore();
        const g = c.createRadialGradient(ox, oy, 0, ox, oy, u*5.5);
  ...
  // ---- O SANTUÁRIO DOS DOZE: mármore negro, ouro e as doze luzes ----
    if(mapName === 'jardim_templo'){
      ...
      if(dx > -TS*8 && dx < canvas.width + TS*8 && base > -TS*12 && base < canvas.height + TS*4){
        c.fillStyle = '#8f887c';                               // escadaria de mármore
        for(let s = 0; s < 2; s++)
          c.fillRect(dx - TS*(2.0 - s*0.4), base + s*4, TS*(4.0 - s*0.8), 4);
        const tw = TS*5.2, th2 = TS*2.3;                       // o TAMBOR negro
        c.fillStyle = '#221c2e';
        c.fillRect(dx - tw/2, base - th2, tw, th2);
        const sh = c.createLinearGradient(dx - tw/2, 0, dx + tw/2, 0);
        sh.addColorStop(0, 'rgba(255,255,255,0.10)'); sh.addColorStop(0.35, 'rgba(255,255,255,0)');
        sh.addColorStop(0.8, 'rgba(0,0,0,0.25)'); sh.addColorStop(1, 'rgba(0,0,0,0.30)');
        c.fillStyle = sh; c.fillRect(dx - tw/2, base - th2, tw, th2);
        c.fillStyle = '#c9a842';                               // frisos dourados
        c.fillRect(dx - tw/2, base - th2, tw, 2);
        c.fillRect(dx - tw/2, base - 3, tw, 3);
        const CORES = _DEUS_CORES || ['#f2c14e','#5a5a6a','#7a8a5a','#e0865a','#e05a4e','#a83838','#8a7ae0','#f2e05a','#b06ae0','#e08ae0','#8a6a3a','#7ac06a'];
        for(let i = 0; i < 12; i++){                           // 12 nichos, um por deus
          const nx = dx - tw/2 + tw*(i + 0.5)/12;
          c.save(); c.globalCompositeOperation = 'lighter';
          c.globalAlpha = 0.85 + 0.15*Math.sin(now/500 + i);
          c.fillStyle = CORES[i % 12];
          c.beginPath(); c.arc(nx, base - th2*0.5, 2.6, Math.PI, 0);
          c.rect(nx - 2.6, base - th2*0.5, 5.2, 6.5); c.fill();
          c.restore();
        }
        c.fillStyle = '#2a2438';                               // A CÚPULA negra
        c.beginPath(); c.ellipse(dx, base - th2, TS*2.7, TS*2.05, 0, Math.PI, 0); c.fill();
        const dg = c.createRadialGradient(dx - TS*0.9, base - th2 - TS*1.3, 0, dx, base - th2 - TS*0.6, TS*2.9);
        dg.addColorStop(0, 'rgba(150,140,180,0.5)'); dg.addColorStop(1, 'rgba(0,0,0,0)');
        c.fillStyle = dg;
        c.beginPath(); c.ellipse(dx, base - th2, TS*2.7, TS*2.05, 0, Math.PI, 0); c.fill();
        c.strokeStyle = 'rgba(201,168,66,0.85)'; c.lineWidth = 1.6;
        for(const k of [-0.66, -0.33, 0, 0.33, 0.66]){         // gomos DOURADOS
          c.beginPath();
          c.moveTo(dx + k*TS*2.7, base - th2);
          c.quadraticCurveTo(dx + k*TS*1.5, base - th2 - TS*2.05, dx, base - th2 - TS*2.05);
          c.stroke();
        }
        for(let i = 0; i < 12; i++){                           // as 12 GEMAS no anel
          const a = Math.PI + i*Math.PI/11;
          const gx = dx + Math.cos(a)*TS*2.55, gy = base - th2 + Math.sin(a)*TS*1.9;
          c.save(); c.globalCompositeOperation = 'lighter';
          c.globalAlpha = 0.8 + 0.2*Math.sin(now/400 + i);
          c.fillStyle = CORES[i % 12];
          c.beginPath(); c.arc(gx, gy, 2.6, 0, Math.PI*2); c.fill();
          c.restore();
        }
        const ly = base - th2 - TS*2.05;                       // coroa: estrela dos Doze
        c.fillStyle = '#c9a842'; c.fillRect(dx - 3.4, ly - 8, 6.8, 8);
        c.save(); c.translate(dx, ly - 15); c.rotate(now/6000);
        c.fillStyle = '#e8c860';
        c.beginPath();
        for(let i = 0; i < 24; i++){
          const R = (i % 2 === 0) ? 8 : 3.4;
        ...
        const hg = c.createRadialGradient(dx, ly - 15, 0, dx, ly - 15, TS*1.5);
        hg.addColorStop(0, 'rgba(255,224,138,0.9)'); hg.addColorStop(1, 'rgba(0,0,0,0)');
        c.fillStyle = hg; c.beginPath(); c.arc(dx, ly - 15, TS*1.5, 0, Math.PI*2); c.fill();
        for(let i = 0; i < 6; i++){                            // motas das doze cores orbitando
          const t = (now/3000 + i/6) % 1;
          const a = t*Math.PI*2;
          c.globalAlpha = 0.6;
          c.fillStyle = CORES[(i*2) % 12];
          c.beginPath(); c.arc(dx + Math.cos(a)*TS*3.1, base - th2 - TS*0.6 + Math.sin(a)*TS*1.1, 1.8, 0, Math.PI*2); c.fill();
        }
        c.restore();
        c.fillStyle = '#c9a842';                               // portal dourado em arco
        c.beginPath(); c.arc(dx, base - 8, 8.5, Math.PI, 0);
        c.rect(dx - 8.5, base - 8, 17, 8); c.fill();
        c.fillStyle = '#171223';
        c.beginPath(); c.arc(dx, base - 7, 6, Math.PI, 0);
        c.rect(dx - 6, base - 7, 12, 7); c.fill();
        for(const bx of [-1.6, 1.6]){                          // braseiros azulados
          const bxx = dx + bx*TS;
          c.fillStyle = '#3a3a46'; c.fillRect(bxx - 2.4, base - 5, 4.8, 6);
          c.save(); c.globalCompositeOperation = 'lighter';
          c.globalAlpha = 0.7 + 0.3*Math.sin(now/240 + bx);
          c.fillStyle = '#8ab0ff';
          c.beginPath(); c.moveTo(bxx, base - 13);
          c.quadraticCurveTo(bxx + 3.4, base - 8.5, bxx, base - 5);
          c.quadraticCurveTo(bxx - 3.4, base - 8.5, bxx, base - 13); c.fill();
          c.restore();
        }
      }
    ...
    if(mapName === 'templo_estrelado'){
      ...
      for(let i = 0; i < 12; i++){                             // as 12 luzes dos altares
        ...
        const lx = (16.5 + 12.4*Math.cos(a))*TS - camX, ly = (15.5 + 12.4*Math.sin(a))*TS - camY;
        c.save(); c.globalCompositeOperation = 'lighter';
      ...
      const ax = 16.5*TS - camX, ay = 15.5*TS - camY;          // o círculo do Arcebispo
      c.save(); c.globalCompositeOperation = 'lighter';
      c.globalAlpha = 0.4;
...
var _PORTAS_VIVAS = [
  ...
  {mapa: 'prospera',   cx: 67, cy: 4,  w: 6, h: 4, emoji: '🍽️'},
  {mapa: 'prospera',   cx: 13, cy: 4, w: 5, h: 3, emoji: '💈'},
  {mapa: 'prospera',   cx: 13, cy: 44, w: 5, h: 3, emoji: '🏠'},
  {mapa: 'prospera',   cx: 50, cy: 44, w: 5, h: 3, emoji: '🏠'},
  {mapa: 'prospera',   cx: 59, cy: 44, w: 6, h: 4, emoji: '🏠'},
  {mapa: 'prospera',   cx: 68, cy: 44, w: 5, h: 3, emoji: '🏠'},
  {mapa: 'prospera',   cx: 4, cy: 53, w: 5, h: 3, emoji: '🏠'},
  {mapa: 'prospera',   cx: 13, cy: 53, w: 6, h: 3, emoji: '🏠'},
  {mapa: 'prospera',   cx: 50, cy: 53, w: 5, h: 3, emoji: '🏠'},
  {mapa: 'prospera',   cx: 59, cy: 53, w: 6, h: 3, emoji: '🏠'},
  {mapa: 'prospera',   cx: 68, cy: 53, w: 5, h: 3, emoji: '🏠'},

  {mapa: 'prospera',   cx: 21, cy: 4, w: 5, h: 4, emoji: '📚'},
  {mapa: 'prospera',   cx: 59, cy: 4, w: 5, h: 3, emoji: '🌸'},
  {mapa: 'prospera',   cx: 76, cy: 4, w: 6, h: 3, emoji: '🧵'},
  {mapa: 'prospera',   cx: 4, cy: 12, w: 5, h: 3, emoji: '🐟'},
  {mapa: 'prospera',   cx: 13, cy: 12, w: 6, h: 4, emoji: '🧀'},
  {mapa: 'prospera',   cx: 67, cy: 12, w: 5, h: 3, emoji: '🍬'},
  {mapa: 'prospera',   cx: 4, cy: 20, w: 5, h: 3, emoji: '🍵'},
  {mapa: 'prospera',   cx: 22, cy: 20, w: 5, h: 3, emoji: '🎻'},
  {mapa: 'prospera',   cx: 4, cy: 44, w: 6, h: 4, emoji: '🗞️'},
  {mapa: 'prospera',   cx: 76, cy: 11, w: 6, h: 4, emoji: '🦚'},
  {mapa: 'prospera',   cx: 22, cy: 44, w: 6, h: 4, emoji: '🏛️'},
  {mapa: 'prospera',   cx: 77, cy: 44, w: 5, h: 4, emoji: '🔨'},
  ...
  drawIlhaDecor = function(c, now){
    ...
    if(mapName === 'ermo'){
      ...
      if(sx > -TS*10 && sx < canvas.width + TS*10 && base > -TS*12 && base < canvas.height + TS*6){
        ...
        for(let s = 0; s < 3; s++)
          c.fillRect(sx - TS*(3.4 - s*0.5), base + TS*1.1 + s*4, TS*(6.8 - s*1.0), 4);
        for(const gx of [-4.2, 4.2]){                          // guardiões de pedra
          const gxx = sx + gx*TS, gy = base + TS*1.1;
          c.fillStyle = '#8f887c';
        ...
        c.fillStyle = '#f0ebdf';                               // A FACHADA BRANCA (até o chão!)
        c.fillRect(sx - TS*7.8, base - TS*5.8, TS*15.6, TS*6.9);
        c.fillStyle = 'rgba(0,0,0,0.07)';
        c.fillRect(sx + TS*2.6, base - TS*5.8, TS*5.2, TS*6.9);
        ...
        for(const pdx of [-1, 0, 1]){                          // PORTAL TRIPLO dourado
          const pxx = sx + pdx*TS*1.15;
          c.fillStyle = '#c9a842';
          c.beginPath(); c.arc(pxx, base + TS*0.25, TS*0.44, Math.PI, 0);
          c.rect(pxx - TS*0.44, base + TS*0.25, TS*0.88, TS*0.75); c.fill();
          c.fillStyle = '#241c30';
          c.beginPath(); c.arc(pxx, base + TS*0.32, TS*0.32, Math.PI, 0);
          c.rect(pxx - TS*0.32, base + TS*0.32, TS*0.64, TS*0.68); c.fill();
          c.save(); c.globalCompositeOperation = 'lighter';
          c.globalAlpha = 0.30 + 0.10*Math.sin(now/500 + pdx);
          const pg = c.createRadialGradient(pxx, base + TS*0.7, 0, pxx, base + TS*0.7, TS*0.8);
          pg.addColorStop(0, 'rgba(255,220,140,0.8)'); pg.addColorStop(1, 'rgba(0,0,0,0)');
          c.fillStyle = pg; c.beginPath(); c.arc(pxx, base + TS*0.7, TS*0.8, 0, Math.PI*2); c.fill();
          c.restore();
        }
